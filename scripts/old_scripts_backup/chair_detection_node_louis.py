...
class ChairDetection():
    ...
    def __init__(self , model_name, cameras: nc.NaoqiCameras, VISUAL) -> None:
        ...
        self.iou_threshold_seat = 0.1
        self.mid_point_seat_person_margin = 30
        ...
        rospy.loginfo(
            bcolors.CYAN+"[RoboBreizh - Vision]    Loading Chair Detection weights done"+bcolors.ENDC)
        
        if self.VISUAL: 
            self.bridge = CvBridge()
            self.pub_opencv = rospy.Publisher('/roboBreizh_detector/chair_detection_image', Image, queue_size=10)
