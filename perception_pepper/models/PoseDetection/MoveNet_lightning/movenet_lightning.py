...
class Movenet(object):
  ...
  def __init__(self, pose_model_name: str = 'movenet_lightning', width: int = 320, height: int = 240) -> None:
    ...
    _, ext = os.path.splitext(pose_model_name)
    if not ext:
      pose_model_name += '.tflite'

    # Initialize the TFLite model.
    base_path = get_pkg_path()
    path = os.path.join(base_path, "scripts/models/PoseDetection/MoveNet_lightning/tensorflow_lite_lighning/")
    ...
    self.image_feature = np.empty(0)
    
  def pre_process(self, image_height: int,
  ...
  def post_process(self, keypoints: np.ndarray, image_height: int,
                             image_width: int) -> Dict[(str, float)]:
    ...
    if self._torso_visible(keypoints):
      ...
      if crop_length_half > max(image_width, image_height) / 2:
        return self.pre_process(image_height, image_width)
  ...
  def _run_detector(
    ...
    self.image_feature = input_image
  ...
  def inference(self,
             ...
             reset_crop_region: bool = False) -> PersonPose:
    ...
    if (self._crop_region is None) or reset_crop_region:
      ...
      self._crop_region = self.pre_process(image_height, image_width)
    ...
    self._crop_region = self.post_process(keypoint_with_scores,
