...
class MoveNetMultiPose(object):
    ...
    def __init__(self,
                 pose_model_name: str = 'movenet_multipose_tflite',
        ...
        path = os.path.join(base_path, "models/PoseDetection/MoveNet_MultiPose/tensorflow_lite_multipose/")
    ...
    def inference(self,
               ...
               detection_threshold: float = 0.11) -> List[PersonPose]:
        ...
        if is_dynamic_shape_model:
            resized_image, _ = keep_aspect_ratio_resizer(
    ...
    def _postprocess(self, keypoints_with_scores: np.ndarray, image_height: int,
                     image_width: int,
                     detection_threshold: float) -> List[PersonPose]:
        ...
        for idx in range(num_instances):
            ...
            list_persons.append(PersonPose(keypoints, bounding_box, person_score))
        if self._tracker:
