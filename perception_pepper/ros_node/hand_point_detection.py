...
class HandPointingDetection():
    ...
    def __init__(self , model_name, cameras: nc.NaoqiCameras, VISUAL) -> None:
        ...
        if self.VISUAL: 
            ...
            self.pub_opencv = rospy.Publisher('/roboBreizh_detector/hand_pointing_image', Image, queue_size=10)
            self.pub_compressed_img = rospy.Publisher("/roboBreizh_detector/hand_pointing_compressed_image",
            CompressedImage,  queue_size=10)
        
        self.initHandPointingDescriptionService()
        
    def initHandPointingDescriptionService(self):
        rospy.Service('/robobreizh/perception_pepper/hand_pointing',
                        pointing_hand_detection, self.handle_ServiceHandPointing)
            
        rospy.loginfo(
            bcolors.O+"[RoboBreizh - Vision]        Starting Hand Pointing Detection. "+bcolors.ENDC)
        rospy.spin()

    def handle_ServiceHandPointing(self, pointing_hand_detection):
        
        rightList = []
        topList = []
        
        self.distanceMax = pointing_hand_detection.distance_max
            ...
            for person in list_person:
                if person.score < self.person_threshold:
                    ...
                    if dist <= self.distanceMax:
                        ...
                        rospy.loginfo(bcolors.R+"[RoboBreizh - Pose/Hand Pointing] Person Detected but not within Range..."+bcolors.ENDC)
                
                rospy.loginfo(bcolors.ON_PURPLE+"[RoboBreizh - Pose/Hand Pointing] detection done..."+bcolors.ENDC)
        else:
            rospy.loginfo(bcolors.R+"[RoboBreizh - Pose/Hand Pointing] No Person Detected"+bcolors.ENDC)
...
if __name__ == "__main__":
    ...
    rospy.init_node('hand_pointing_node', anonymous=True)
    ...
    HandPointingDetection("movenet_multipose", cameras, VISUAL)
